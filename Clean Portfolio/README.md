...
The archived React theme at `/old-themes/` is kept out of search results: the build rewrites its `robots` meta to `noindex, follow`, so the current design is the one that ranks for the name. The sitemap lists only the four pages that should rank; the three article pages are left out because they point their canonical at the blog. `Clean Portfolio/827e7df31e49331a435d8b7a0f6416fb.txt` is the public IndexNow key: it proves this domain's identity to Bing, Yandex, Seznam, and Naver, and the Pages workflow tells them about each deploy. Google has no equivalent ping, so its sitemap is submitted once in Search Console by hand.
