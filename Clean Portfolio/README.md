...
Pages are saved before they are opened. `sw.js` caches the shared shell on install and then serves each navigation from that cache while quietly checking for a newer copy, so repeat visits paint immediately and work offline. `site.js` warms the links a visitor is most likely to use: the navigation, footer, and page links are saved during idle time after load, and any link that is hovered, tapped, or focused is saved right away, roughly 100–400 ms before the click lands. Requests to the blog, booking, and GitHub hosts are never intercepted or stored, live visitor and contribution requests always go to the network, and a blocked or unsupported service worker merely means every link loads normally. Because `sw.js` lives at the site root it must stay in the build's copy list in `scripts/build-site.mjs`; its `CACHE_VERSION` only needs a bump when the shell list or caching behaviour itself changes.
